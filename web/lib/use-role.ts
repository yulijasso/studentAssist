...
export function useRole() {
  ...
  return {
    ...
    isInstitutionAdmin: roleName === "institution_admin",
    ...
    canAssign: ["tech_admin", "institution_admin", "supervisor"].includes(roleName ?? ""),
    /** Whether the user can change SLA/settings (admin only). */
    canAdmin: ["tech_admin", "institution_admin"].includes(roleName ?? ""),
  };
}

export function useIsAdmin() {
  const { role } = useRole();
  return role === "tech_admin" || role === "institution_admin";
}
